...
X_data_all_rand, Y_lable_all_rand = loadDataFromCSV('/Users/Chris/Python/Machine Learning/Masterarbeit1/all Data/Data Archiv/ExposureData05052017maturities6.csv', True)
OtherTestData_rand, OtherTestLables_rand = loadDataFromCSV('/Users/Chris/Python/Machine Learning/Masterarbeit1/all Data/Data Archiv/ExposureData05052017maturities6Test.csv', True)
...
with tf.Session() as sess:
    ...
    TestAtEnd, correct_pred_testAtEnd = sess.run([accuracy, correct_pred], feed_dict={x: X_data_test, y: X_lables_test, keep_prob: 1.})
    print("Testing Accuracy:", TestAtEnd)

    OtherTest, correct_pred_otherTest = sess.run([accuracy, correct_pred], feed_dict={x: OtherTestData_rand, y: OtherTestLables_rand, keep_prob: 1.})
...
for i in range(0, n_classes):
    correct = sum(1 for k in range(0, X_lables_test.shape[0]) if (correct_pred_testAtEnd[k] == True and X_lables_test[k,i] == 1))
    number = sum(np.transpose(X_lables_test)[i])
    print("class ", i, ": ", number, "\t correct: ", correct, "\t", float(np.round(correct/number*100, 3)), "%")    

print("Number of representations in classes (other test data)")
for i in range(0, n_classes):
    correct = sum(1 for k in range(0, OtherTestLables_rand.shape[0]) if (correct_pred_otherTest[k] == True and OtherTestLables_rand[k,i] == 1))
    number = sum(np.transpose(OtherTestLables_rand)[i])
    print("class ", i, ": ", number, "\t correct: ", correct, "\t", float(np.round(correct/number*100, 3)), "%")    
