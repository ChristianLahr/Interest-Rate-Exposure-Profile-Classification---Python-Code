...
def loadDataFromCSV(path, normalize, maturityStyle, randomize): # normalize = True --> normalization over altitude of every row (=time series)
    ...
    lables = completeData.iloc[:, completeData.shape[1] - 4 : completeData.shape[1] - 3 ].values
...
timeSeriesLength = 120  # time series data input shape: 120 data points
n_classes = 15
...
X_data_all_rand, Y_lable_all_rand = loadDataFromCSV(file1Name, True, maturityStyle, False)
...
with tf.name_scope('summaries'):
    tf.summary.histogram('hier', weights['wc1'])
merged = tf.summary.merge_all()
                           
# Launch the graph
with tf.Session() as sess:
    train_writer = tf.summary.FileWriter('/Users/Chris/Python/Machine Learning/Masterarbeit1/CNN/savedDataForTensorBoard/logs', sess.graph)
    sess.run(init)
