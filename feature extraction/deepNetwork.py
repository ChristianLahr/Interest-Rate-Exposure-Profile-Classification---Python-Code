...
"""
maximum, maximumPosition, entryPoint, numberOfZeros, mean, variance, maximalDistanceToTheNextDrop, deltaMaxFist, numberOfJumpsRelativeToLength, maximalJump, averageJump, jumps, 
delta, timeAfterJump, numberOfJumps, 
...
networkInput = [maximum, maximumPosition, entryPoint, numberOfZeros, mean, variance, maximalDistanceToTheNextDrop, deltaMaxFist, numberOfJumpsRelativeToLength, maximalJump, averageJump, jumps, probabilities_Maturity, probabilities_Frequency, probabilities_Coupon, labels_maturity, labels_frequency, labels_coupon]
networkInput_length_oneDImFeatures = networkInput_length
...
#    maximum, maximumPosition, entryPoint, deltaMaxFist, delta, jumps, timeAfterJump, numberOfZeros, maximalDistanceToTheNextDrop, mean, variance, numberOfJumps, averageJump, numberOfJumpsRelativeToLength, maximalJump = sess.run(optimizer, feed_dict={inputData_tf_oneDimFeatures: batch_x_1})
    ...
    while step * batch_size < training_iters:
        ...
        if((step - r * stepsToRunThroughTrainingsData) > stepsToRunThroughTrainingsData):
            ...
            networkInput, labels_notUsed = randomizeRowsOfArraysInList(networkInput, labels_maturity)
            networkInput_oneDimFeatures = np.array([networkInput[0], networkInput[1], networkInput[2], networkInput[3], networkInput[4], networkInput[5], networkInput[6], networkInput[7], networkInput[8], networkInput[9], networkInput[10]])
            networkInput_oneDimFeatures = np.transpose(networkInput_oneDimFeatures)
            networkInput_multiDimFeature_1 = networkInput[networkInput_length_oneDImFeatures]
            networkInput_prob_Maturity = networkInput[networkInput_length_oneDImFeatures + 1]
