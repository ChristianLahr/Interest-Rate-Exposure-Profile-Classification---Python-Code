...
import pandas as pd
import numpy as np
from ownFeatureExtractionModels import Model_easy
...
init, optimizer, cost, accuracy, inputData_tf, lables_tf, keep_prob_tf, weights, biases, prediction = Model_easy(networkInput_length, n_classes, numberWeigthsPerNeuronInFirstLayer, learning_rate)
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        ...
        sess.run(optimizer, feed_dict={inputData_tf: batch_x, 
                                       lables_tf: batch_y,
                                       keep_prob_tf: dropout})
        if step % display_step == 0:
            # Calculate batch loss and accuracy
            loss, acc = sess.run([cost, accuracy], feed_dict={  inputData_tf: batch_x, 
                                                                lables_tf: batch_y,
                                                                keep_prob_tf: 1.})
    ...
    overallTest = float(sess.run(accuracy, feed_dict={inputData_tf: networkInput,
                                                    lables_tf: lables,
                                                    keep_prob_tf: 1.}))
